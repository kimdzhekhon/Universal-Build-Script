...
3.2에서는 `build.sh`를 얇은 진입점으로 유지하면서 Python이 Node workspace·lockfile 감지, 충돌 그룹 직렬화, 실행 옵션을 포함한 plan을 담당합니다. 설치기는 전체 release를 staging·SHA-256 검증한 뒤 원자 교체하고, Bash는 Flutter/Tauri 플랫폼 계층, 선택형 Rust helper는 update manifest 일괄 검증을 담당합니다.
...
> **2.x에서 3.x로 올릴 때:** 2.x updater는 Python/Rust 관리 파일을 받을 수 없으므로 위 `UBS_FORCE=true` 설치 명령을 한 번 사용해야 합니다. 3.x 설치가 끝난 뒤에는 `./build.sh update`가 전체 24개 관리 파일을 갱신합니다.

설치기는 기본적으로 `v3.2.0` release ref의 manifest와 24개 파일을 모두 staging·검증한 뒤 한 트랜잭션으로 적용합니다. 다른 불변 ref를 검증할 때만 `UBS_INSTALL_REF`를 지정하십시오.
...
├── scripts/
│   ├── ubs.py                       # Python 감지·계획·병렬 실행·Node·Gradle 코어
...
- 완전한 생태계 의존성 그래프와 위상 정렬은 아직 제공하지 않으므로 별도 workspace 간 암묵적 의존성은 사용자가 확인해야 합니다.
