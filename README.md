...
3.0부터 `build.sh`는 Python 코어를 호출하는 얇은 호환 진입점입니다. 3.1에서는 Node·Gradle 실행, 의존성 입력 캐시, 제한 병렬 스케줄링까지 Python으로 이동했습니다. Bash는 Flutter/Tauri 플랫폼 처리와 설치·복구 호환 계층을 맡고, 선택형 Rust helper는 업데이트 manifest를 한 프로세스에서 일괄 비교·검증합니다.
...
| Node 캐시 | package/lock 입력이 같으면 의존성 재설치 생략 |
...
Python 코어가 감지한 패키지 매니저로 의존성을 설치한 다음 `scripts.build`를 실행합니다. 기본 `UBS_INSTALL_MODE=auto`는 `package.json`과 lockfile 해시가 같고 이전 설치가 성공했다면 install 단계를 생략합니다.
