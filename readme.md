...
Flutter 프로젝트 루트에서 아래 한 줄을 실행하면 `scripts/build.sh`, `.env.example`, `ios/ExportOptions.plist`가 자동으로 생성됩니다.

```bash
curl -fsSL https://raw.githubusercontent.com/kimdzhekhon/Flutter-Optimization-Build-Script/main/install.sh | bash
```

설치 후 `.env`에 API 키를 채우고 빌드를 시작하세요.
...
├── ios
│   └── ExportOptions.plist  # iOS 배포 설정 템플릿
