...
| **알림** | **Build Notification** | 빌드 완료 시 사운드, 음성, 시스템 배너 알림 (macOS) |
...
빌드 완료 후 macOS 환경에서는 해당 폴더가 Finder에서 자동으로 열립니다.
...
1. **핵심 문제** — 파일 전체를 감싸고 있던 ` ```markdown ``` ` 코드블록 제거 (이게 깨짐의 원인)
2. "권한 부여" 섹션에서 코드블록 위에 중복으로 적혀 있던 `chmod +x scripts/build.sh` 텍스트 제거
